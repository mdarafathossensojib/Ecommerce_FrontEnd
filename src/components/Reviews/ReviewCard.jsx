...
const ReviewCard = ({ review, user, editReview, setEditReview, onEditClick, isEditing, onCancelEdit, onSave, onDelete }) => {
    return (
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    ...
                    {user && user.id === review.user.id && (
                        <div className="flex gap-2">
                            <button className="btn btn-sm btn-outline btn-primary" onClick={onEditClick}>Edit</button>
                            <button className="btn btn-sm btn-outline btn-error" onClick={onDelete}>Delete</button>
                        </div>
                    )}
                </div>
                {isEditing ? (
                    <EditReviewForm editReview={editReview} setEditReview={setEditReview} onCancelEdit={onCancelEdit} onSave={onSave} />) : (
                    <div className="mt-4">
